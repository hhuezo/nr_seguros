...
return new class extends Migration
{
    ...
    {
        Schema::create('poliza_desempleo_detalle_preliminar', function (Blueprint $table) {
            $table->bigIncrements('Id');
            $table->unsignedBigInteger('PolizaDesempleoId');
            $table->integer('Axo');
            $table->integer('Mes');
            ...
            $table->index('PolizaDesempleoId');
            $table->index(['Axo', 'Mes']);

            // Foreign key si existe la tabla poliza_desempleo
            $table->foreign('PolizaDesempleoId')->references('Id')->on('poliza_desempleo')->onDelete('restrict');
        });
    }
    ...
    {
        Schema::dropIfExists('poliza_desempleo_detalle_preliminar');
    }
};
