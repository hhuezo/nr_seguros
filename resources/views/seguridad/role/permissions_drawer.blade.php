<!-- Overlay del drawer -->
<div class="drawer-overlay" id="permissionsDrawerOverlay_{{ $rol->id }}" onclick="closePermissionsDrawer_{{ $rol->id }}({{ $rol->id }})"></div>

<!-- Drawer para Asignar Permisos -->
<div class="drawer drawer-permissions" id="permissionsDrawer_{{ $rol->id }}">
    <div class="drawer-header">
        <h4>Asignar Permisos</h4>
        <button type="button" class="drawer-close" onclick="closePermissionsDrawer_{{ $rol->id }}({{ $rol->id }})">
...
<script>
    function openPermissionsDrawer_{{ $rol->id }}(roleId) {
        document.getElementById('permissionsContainer_{{ $rol->id }}').innerHTML = '<p class="text-center text-muted">Cargando permisos...</p>';
        ...
        .then(data => {
            if (data.success) {
                ...
                document.getElementById('permissionsContainer_{{ $rol->id }}').innerHTML = '<p class="text-center text-danger">Error al cargar los permisos</p>';
            }
        })
        .catch(error => {
