...
class RehabilitadosExport implements FromCollection, WithHeadings
{
    /**
   ...
   protected $id;
    ...
    public function collection()
    {
        ...
        if ($deuda->Aseguradora == 3 || $deuda->Aseguradora == 4) {
            $data = PolizaDeudaTempCartera::where('poliza_deuda_temp_cartera.PolizaDeuda', $this->id)
                ...
                ->get();
            // Formato anterior Fedecrédito (por si deciden volver):
            // $data = PolizaDeudaTempCartera::where('poliza_deuda_temp_cartera.PolizaDeuda', $this->id)
            //     ->where('Rehabilitado', 1)
            //     ->join('saldos_montos as sm', ...)->join('poliza_deuda_tipo_cartera as pdtc', ...)->join('tipo_cartera as tc', ...)
            //     ->select(['TipoDocumento','Dui','PrimerApellido','SegundoApellido','PrimerNombre','Nacionalidad','FechaNacimiento','Sexo',
            //     DB::raw("CONCAT(NumeroReferencia, ' ') AS NumeroReferencia"),'FechaOtorgamiento', montos...,'PorcentajeExtraprima','Tasa',
            //     'tc.Nombre as TipoCartera', DB::raw("CONCAT(sm.Abreviatura, ' - ', sm.Descripcion) AS LineaCredito")])->orderBy('NumeroReferencia')->get();
        } else {
            $data = PolizaDeudaTempCartera::where('poliza_deuda_temp_cartera.PolizaDeuda', $this->id)
                ->where('Rehabilitado', 1)
    ...
    public function headings(): array
    {
        ...
        if ($deuda->Aseguradora == 3 || $deuda->Aseguradora == 4) {
            ...
            // Formato anterior Fedecrédito (por si deciden volver):
            // return ['Tipo de documento','DUI o documento de identidad','Primer Apellido','Segundo Apellido','Nombres','Nacionalidad','Fecha de Nacimiento','Género','Nro. de Préstamo','Fecha de otorgamiento','Monto original de desembolso','Saldo de deuda capital actual','Saldo intereses corrientes','Mora capital','Saldo intereses por mora','Intereses Covid','Extra Prima','TARIFA','TIPO CARTERA','LINEA CREDITO'];
        } else {
            // Otras aseguradoras
            return [
                'DUI',
