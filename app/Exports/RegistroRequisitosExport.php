...
class RegistroRequisitosExport implements FromCollection, WithHeadings
{
    ...
    public function collection()
    {
        ...
        // Fedecrédito: solo las 21 columnas acordadas (hasta TARIFA)
        if ($deuda->Aseguradora == 3 || $deuda->Aseguradora == 4) {
            ...
            $data = $data->select([
                    ...
                    DB::raw("IF(COALESCE(poliza_deuda_temp_cartera.MoraCapital, poliza_deuda_temp_cartera.SaldoInteresMora) IS NULL, '', ROUND(COALESCE(poliza_deuda_temp_cartera.MoraCapital, poliza_deuda_temp_cartera.SaldoInteresMora), 2)) AS MoraCapital"),
                ...
                // Formato anterior Fedecrédito (por si deciden volver, reemplazar el select anterior por este):
                // ->select([
                //     'poliza_deuda_temp_cartera.TipoDocumento',
                //     'poliza_deuda_temp_cartera.Dui',
                //     'poliza_deuda_temp_cartera.PrimerApellido',
