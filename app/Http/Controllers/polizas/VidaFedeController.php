...
use App\Imports\VidaCarteraTempCompFedeImport;
...
class VidaFedeController extends Controller
{
    ...
    public function create_pago_recibo(Request $request)
    {
        ...
        // Leer fila a fila y dejar de leer al encontrar la primera vacía o incompleta (no evaluar las 104k filas)
        $sheet = $excel->getActiveSheet();
        $import = new VidaCarteraTempCompFedeImport($request->Axo, $request->Mes, $id, $request->FechaInicio, $request->FechaFinal, $request->PolizaVidaTipoCartera);
        $highestRow = $sheet->getHighestRow();
        for ($rowIndex = 2; $rowIndex <= $highestRow; $rowIndex++) {
            $row = $sheet->rangeToArray('A' . $rowIndex . ':Z' . $rowIndex)[0];
            $model = $import->processRow($row);
